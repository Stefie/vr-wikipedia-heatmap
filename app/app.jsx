...
class VRStream extends React.Component {
  ...
  componentWillMount() {
    this.setState({overlay: <Welcome startFunction={() => this._fetchEdits(true)}/>});
  }
  componentDidMount() {
    const aScene = document.querySelector('a-scene'),
      _this = this;
    if (aScene.isMobile) {
      this.setState({globeSrc: './app/assets/images/natural-earth-small.jpeg'});
    } else {
      this.setState({globeSrc: './app/assets/images/natural-earth.jpg'});
    }
  ...
  _fetchEdits(indicator) {
    ...
    this.eventsource.onmessage = (message) => {
      ...
      if (this.state.tabIsVisible) 
        this._handleStreamData(message);
      };
    this.eventsource.onerror = (error) => {
      this.setState({overlay: <APIError/>});
    }
  }
  _handleStreamData(message) {
    ...
    if (isAnonymous) {
      ...
      if (data.length) {
        let dataLengthOld = data.length.old
          ? data.length.old
          : 0;
  ...
  _getCoordinates(user, radius, color, type, title) {
    ...
    Rp(options).then((location) => {
      //  Add marker if IP lookup was successful
      this.setState(previousState => ({
        storedIPs: [
          ...previousState.storedIPs,
          user
        ],
        ipCoordinates: [
          ...previousState.ipCoordinates, {
            user,
      ...
      this.setState({overlay: <ConnectionError/>});
    });
  }
  _validateIP(user) {
  ...
  _addLocations(lat, lng, radius, color, type, title) {
    ...
    if (this.state.locations.length >= 150) {
      this.setState(previousState => ({
        locations: [
          ...previousState.locations.filter((_, i) => i !== 0), {
            'position': position,
  ...
  render() {
    return (<div className="scene-wrapper">
      ...
      <Scene>
        ...
        <Entity id="vr-wikipedia-heatmap" geometry="primitive: sphere; radius: 30; segmentsWidth: 16; segmentsHeight: 16;" material="src: #globe; repeat: -1 1; side: back; fog: false;" position="0 0 0"></Entity>
